#ifndef _CXXGCTX_WINDOW
#define _CXXGCTX_WINDOW
...
namespace cxxgctx
{
//在friend前声明函数可以保持其链接方式。如果在friend声明前没有显示声明函数，则将其视作extern链接
static void __resize_callback(GLFWwindow* glfw_window, int new_width, int new_height);
...
private:
	...
	resize_callback window_resize_callback = nullptr;
	int width = 800, height = 600;
	bool is_full_screen = false;
	bool visible = true;
	const char* title = nullptr;
	cxxsci::color_rgba back_color;

public:

	window() = default; //该构造函数不会实际初始化窗口仅分配内存，需要重新调用其他构造函数以正常工作
	window(const char* title, int width = 800, int height = 600, cxxsci::color_rgba back_color = {0, 0, 0, 0}, bool is_full_screen = false, GLFWmonitor* monitor = glfwGetPrimaryMonitor());
	 ...
	 * @brief 设置窗口图标
	 */
	inline void set_icon(cxxmmproc::raster_image& icon)
	{
		set_icon((GLFWimage*)icon);
	}

	inline void set_back_color(cxxsci::color_rgba back_color = {0, 0, 0, 0})
	{
		this->back_color = back_color;
	}

	inline cxxsci::color_rgba get_back_color()
	{
		return back_color;
	}

	inline window& set_back_color(cxxsci::color_channel ch, float v)
	{
		back_color.set_rgba(ch, v);
		return *this;
	}

	inline float get_back_color(cxxsci::color_channel ch)
	{
	...
	inline static bool _win_set_window_alpha(HWND handle, cxxsci::pixel transparent_color, int rest_alpha)
	{
	...
	inline static bool _win_set_window_alpha(HWND handle, cxxsci::pixel transparent_color)
	{
