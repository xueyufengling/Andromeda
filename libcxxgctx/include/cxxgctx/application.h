#ifndef _CXXGCTX_APPLICATION
#define _CXXGCTX_APPLICATION

#include <atomic>

#include <cxxcomm/log.h>
#include <cxxcomm/thread.h>
...
//所有cxxgctx::application必须添加的friend class
#define ConstructApplication(_Derived)\
		...
		friend class cxxgctx::application<_Derived>;\
		friend class cxxgctx::application<_Derived>::main_loop_thread;
 ...
 */
template<typename _Derived>
class application : public cxxcomm::sequential_lock
{
	enable_intl_exist_memb_with_type(preinitialize, initialize, update, render_update, terminate)

protected:
	class main_loop_thread: public cxxcomm::thread<void(), main_loop_thread>, public cxxcomm::sequential_lock
	{
	ConstructThread(main_loop_thread, void())

	friend class cxxgctx::application<_Derived>;
	friend class cxxgctx::window_application<_Derived>;

	using cxxcomm::sequential_lock::swap;
	...
	public:
		main_loop_thread(_Derived *derived_app) :
				cxxcomm::thread<void(), main_loop_thread>(&(derived_app->is_running), cxxcomm::thread_work_mode::DETACH)
		{
			app = derived_app;
		}
		...
		using cxxcomm::thread<void(),main_loop_thread>::start;
		using cxxcomm::thread<void(),main_loop_thread>::stop;
	};
