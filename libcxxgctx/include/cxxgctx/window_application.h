...
class window_application: public cxxgctx::window, public cxxgctx::application<_Derived>
{
	ConstructApplication(_Derived)

private:
	using cxxgctx::application<_Derived>::is_running;
	using cxxgctx::application<_Derived>::synchronize_fps;
	using cxxgctx::application<_Derived>::app_main_loop_thread;
	frame_rate render_frame_rate; //渲染循环计数器
	andromeda::graphics::framebuffer framebuffer; //双缓冲
	int render_fps_limit = cxxgctx::frame_rate::UNLIMITED;
	int update_rate_limit = cxxgctx::frame_rate::UNLIMITED;

	using cxxgctx::application<_Derived>::_initialize;
	using cxxgctx::application<_Derived>::_preinitialize;
	using cxxgctx::application<_Derived>::_terminate;
	using cxxgctx::application<_Derived>::_update;
	using cxxgctx::application<_Derived>::_render_update;
	using cxxgctx::application<_Derived>::swap;
...
public:
	...
	__attribute__((always_inline)) inline void set_synchronize_fps(bool synchronize_fps_)
	{
		...
		if(synchronize_fps_)
			app_main_loop_thread->set_update_rate_limit(cxxgctx::frame_rate::UNLIMITED); //开启了帧率同步，则以渲染帧率去同步更新率
		else
	...
	__attribute__((always_inline)) inline void set_render_fps_limit(int fps_limit)
	{
		...
		if(synchronize_fps)
			app_main_loop_thread->set_update_rate_limit(cxxgctx::frame_rate::UNLIMITED);
	}
	...
	__attribute__((always_inline)) inline void set_update_rate_limit(int ur_limit)
	{
		...
		if(synchronize_fps)
			render_frame_rate.set_fps_limit(cxxgctx::frame_rate::UNLIMITED);
	}
...
#endif//_CXXGCTX_WINDOWAPPLICATION