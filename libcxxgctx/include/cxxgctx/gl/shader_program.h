#ifndef _CXXGCTX_GL_SHADERPROGRAM
#define _CXXGCTX_GL_SHADERPROGRAM
...
public:
	...
	{
		return cxxgctx::gl::gl_get_integer(GL_CURRENT_PROGRAM);
	}
...
//适用于偶尔设置变量值（glGetUniformLocation查询代价高昂避免循环调用！），设置前后不改变当前着色器程序
	...
	void set(const char* name, int count, const cxxsci::matrix3x3f* value_arr, bool transpose = false);
	void set(const char* name, const bool value); //调用setUnsignedInt设置为0或1
	void set(const char* name, const cxxsci::vector3f vec3);
	void set(const char* name, const cxxsci::matrix3x3f& mat3, bool transpose = false);
	void set(const char* name, const cxxsci::matrix3x3f* mat3, bool transpose = false);
...
//适用于频繁设置变量值，使用前使用use()设定着色器后才可以调用set()和setArray()
	...
	public:
		...
		inline void set(const cxxsci::vector3f vec3)
		{
			gl_Uniform1fv(var_loc, 3, (const GLfloat* )&vec3);
		}

		inline void set(const cxxsci::matrix3x3f& mat3, bool transpose = false)
		{
...
#endif //_CXXGCTX_GL_SHADERPROGRAM
