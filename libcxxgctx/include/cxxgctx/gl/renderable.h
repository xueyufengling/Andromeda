#ifndef _CXXGCTX_GL_RENDERABLE
#define _CXXGCTX_GL_RENDERABLE
...
#include <cxxgctx/gl/gl_basic.h>
#include <cxxgctx/gl/texture_2d.h>
#include <cxxgctx/gl/vertex_attribute.h>
...
class renderable_instance
{
	...
	private:
	cxxsci::matrix4x4f model_matrix = cxxsci::matrix4x4f::identity(); //变换数据
...
public:
	...
	{
		model_matrix = cast<cxxsci::matrix4x4f>(model_matrix * cxxsci::scale(scale_factor));
		return *this;
	}

	inline renderable_instance& shift(cxxsci::vector3f shift)
	{
		model_matrix = cast<cxxsci::matrix4x4f>(model_matrix * cxxsci::shift(shift));
		return *this;
	}

	inline renderable_instance& rotate(float rad_x, float rad_y, float rad_z)
	{
		model_matrix = cast<cxxsci::matrix4x4f>(model_matrix * cxxsci::Rxyz(rad_x, rad_y, rad_z));
		return *this;
	}

	inline renderable_instance& rotate(cxxsci::vector3f axis, float rad)
	{
		model_matrix = cast<cxxsci::matrix4x4f>(model_matrix * cxxsci::R(axis, rad));
		return *this;
	}
...
public:
	...
	cxxsci::matrix4x4f vertex_pretransform = cxxsci::matrix4x4f::identity(); //加载模型后需要进行的变换
