...
unsigned char* cxxcomm::read(const std::string filename, size_t* data_length, long long int read_length, size_t reserve_length)
{
...
std::string cxxcomm::read_string(const std::string filename, long long int read_length, size_t reserve_length)
{
	size_t str_length = 0;
	char* str = (char*)cxxcomm::read(filename, &str_length, read_length, reserve_length + 1);
	str[str_length] = '\0';
	return str;
}

void cxxcomm::write(const std::string filename, const void* data, size_t length)
{
...
void cxxcomm::append(const std::string filename, const void* data, size_t length)
{
...
void cxxcomm::append_string(const std::string filename, const std::string& str)
{
...
void cxxcomm::append_string_newline(const std::string filename, const std::string& str)
{
	std::ofstream file(u8path(filename).generic_u8string(), std::ios::app);
	file << '\n' << str;
	file.close();
...
void cxxcomm::clear(const std::string file_name)
{
...
std::string cxxcomm::directory_of(const std::string file_path)
{
