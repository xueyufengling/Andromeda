...
#include <iostream>
static unwind_action _unwind_ip_op(_Unwind_Context* context, void* _call_stack_ptr, int step_depth, int frame_depth)
{
	...
	std::cout << std::dec << frame_depth << " -> " << std::hex << (_call_stack->instruction_pointers[frame_depth]) << std::endl;
